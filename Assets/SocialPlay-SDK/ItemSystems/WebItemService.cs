...
public class WebItemService : IItemService
{
    ...
    public void ChangeStackAmount(Guid StackRemove, int amount, Action<string> callback)
    {
        string url = string.Format("{0}ChangeStackAmount?stackID={1}&amount={2}", cloudGoodsURL, StackRemove, amount);
        WWWPacket.Creat(url, callback);
    }
