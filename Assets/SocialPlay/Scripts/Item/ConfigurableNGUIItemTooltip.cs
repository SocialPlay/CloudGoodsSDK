        ...
        foreach (DisaplyOption selectedOption in displayOptions)
        {
            switch (selectedOption)
            {
                case DisaplyOption.name:
                    ...
                    {
                        formated += "[" + NGUIText.EncodeColor(ItemQuailityColorSelector.GetColorForItem(item)) + "]";
                    }
                    else
                    {
                ...
                case DisaplyOption.quantity:
                    formated = string.Format("{0}\n[" + NGUIText.EncodeColor(Color.white) + "]{1}", formated, item.stackSize);
                    break;
                case DisaplyOption.description:
                    if (!string.IsNullOrEmpty(item.description))
                    {
                        formated = string.Format("{0}\n[" + NGUIText.EncodeColor(Color.white) + "]{1}", formated, item.description);
                    }
                    break;

                case DisaplyOption.salePrice:
                    formated = string.Format("{0}\n[" + NGUIText.EncodeColor(Color.white) + "]{1}", formated, item.salePrice);
                    break;
                case DisaplyOption.behaviour:
                    foreach (BehaviourDefinition behaviour in item.behaviours)
                    {
                        formated = string.Format("{0}\n[" + NGUIText.EncodeColor(Color.white) + "]{1}", formated, behaviour.Name);
                    }
                ...
                case DisaplyOption.behaviourPlus:
                    foreach (BehaviourDefinition behaviour in item.behaviours)
                    {
                        formated = string.Format("{0}\n[" + NGUIText.EncodeColor(Color.white) + "]{1}: {2}", formated, behaviour.Name, behaviour.Description);
                    }
                    break;
                ...
                case DisaplyOption.energy:
                    formated = string.Format("{0}\n[" + NGUIText.EncodeColor(Color.white) + "]{1}", formated, item.totalEnergy);
                    break;
                case DisaplyOption.stackID:
                    formated = string.Format("{0}\n[" + NGUIText.EncodeColor(Color.white) + "]{1}", formated, item.stackID.ToString());
                    break;
